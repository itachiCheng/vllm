...
class FlexAttentionMetadata:
    ...
    dflash_micro_block_size: int | None = None
    dflash_anchor_len: int = 1
    ...
    def get_paged_mask_mod(self) -> _mask_mod_signature:
        ...
        if self.dflash_micro_block_size is not None:
            block_size = self.dflash_block_size or self.max_query_len
            micro_block_size = self.dflash_micro_block_size
            anchor_len = self.dflash_anchor_len

            def dflash_micro_block_mask_mod(
                b: torch.Tensor,
                h: torch.Tensor,
                q_idx: torch.Tensor,
                physical_kv_idx: torch.Tensor,
                ...
                block_start = self.decode_offset[q_req]
                q_offset = logical_q_idx - block_start
                kv_offset = logical_kv_idx - block_start

                context_visible = logical_kv_idx < block_start
                same_block = (kv_offset >= 0) & (kv_offset < block_size)
                ...
                block_visible = same_block & (
                    anchor_visible | spec_anchor_visible | spec_visible
                )
                return is_valid & (context_visible | block_visible)

            return dflash_micro_block_mask_mod
...
class FlexAttentionMetadataBuilder(AttentionMetadataBuilder[FlexAttentionMetadata]):
    ...
    def build(
        ...
        out = FlexAttentionMetadata(
            ...
            dflash_micro_block_size=common_attn_metadata.dflash_micro_block_size,
            dflash_anchor_len=common_attn_metadata.dflash_anchor_len,
            dflash_block_size=common_attn_metadata.dflash_block_size,
        )
