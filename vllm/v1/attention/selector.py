...
def get_attn_backend(
    ...
    if speculative_config is not None and speculative_config.method == "dflash":
        ...
        if isinstance(dflash_config, dict):
            use_dflash_micro_block = bool(dflash_config.get("micro_block_size"))

    backend = vllm_config.attention_config.backend
    if use_dflash_micro_block:
        from vllm.platforms import current_platform

        if current_platform.is_cuda():
            if backend not in (None, AttentionBackendEnum.FLEX_ATTENTION):
                raise ValueError(
                    "DFlash micro-block attention requires the FLEX_ATTENTION "
                    f"backend on CUDA, got {backend}."
                )
